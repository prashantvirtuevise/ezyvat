...
namespace Easyvat.Services.DataServices
{
    public class PurchaseService
    {
        ...
        public async Task<Purchase> GetInvoiceItems(Guid? id)
        {
        ...
        public async Task<List<PurchaseSummaryReturnModel>> GetNewPurchasesSummary(Guid? memberId)
        {
            List<PurchaseSummaryReturnModel> result = new List<PurchaseSummaryReturnModel>();
            try
            {
                string memId = "910FD9F1-1B66-4650-A229-08D8DC218ABA";
                result = (from pr in ctx.Purchases
                          join sp in ctx.Shops
                          on pr.ShopId equals sp.Id
                          where pr.PassportId.ToString() == memId
